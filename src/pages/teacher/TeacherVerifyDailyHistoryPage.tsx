...
const TeacherVerifyDailyHistoryPage = () => {
    ...
    const loadLogs = async () => {
        setLoading(true);
        try { const res = await api.get(`/advisor/daily-logs/${studentId}`); setLogs(res.data.logs || []); } catch {} finally { setLoading(false); }
    };

    const handleReview = async (logId: number) => {
        if (!comment.trim()) return;
        try { await api.post(`/advisor/daily-logs/${logId}/review`, null, { params: { comment: comment.trim() } }); setComment(''); setReviewingId(null); loadLogs(); } catch {}
    };
                    ...
                    {logs.map((log: any) => (
                        <div key={log.id} className="bg-white rounded-2xl p-6 shadow-md">
                            <div className="flex justify-between items-start mb-3">
                                ...
                                {log.advisor_comment ? <span className="px-3 py-1 rounded-full text-xs font-bold bg-green-50 text-green-600">ตรวจแล้ว</span> : <span className="px-3 py-1 rounded-full text-xs font-bold bg-yellow-50 text-yellow-600">รอตรวจ</span>}
                            </div>
                            <p className="text-gray-700 mb-2"><strong>กิจกรรม:</strong> {log.activities}</p>
                            {log.problems && <p className="text-gray-600 text-sm mb-2"><strong>ปัญหา:</strong> {log.problems}</p>}
                            {log.advisor_comment && <p className="text-blue-600 text-sm mt-2"><strong>ความเห็น:</strong> {log.advisor_comment}</p>}
                            {!log.advisor_comment && (
                                reviewingId === log.id ? (
                                    ...
                                    <button onClick={() => setReviewingId(log.id)} className="mt-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-full text-sm font-bold hover:bg-blue-100">ตรวจ + ให้ความเห็น</button>
                                )
                            )}
                        </div>
                    ))}
