    ...
    return (
        <div className="p-4 md:p-8">
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-3xl font-bold text-[#032B68]">ประเมินนักศึกษา</h1>
                <button
                    onClick={() => { setShowAddModal(true); fetchUnassigned(); }}
            ...
            {loading ? (
                ...
                <div className="text-center py-12 text-gray-400">
                    {students.length === 0 ? 'ยังไม่มีนักศึกษา กดปุ่ม "เพิ่มนักศึกษา"' : 'ไม่พบนักศึกษาที่ค้นหา'}
                </div>
            ) : (
                    ...
                    {filtered.map((s: any) => {
                        ...
                        return (
                            <div key={s.internship_id} className="bg-white rounded-2xl p-6 shadow-md flex justify-between items-center">
                                <div>
                                    <p className="font-bold text-gray-800">{s.student_code} {s.full_name}</p>
                                    <p className="text-sm text-gray-500 mt-1">{s.completed_hours}/{s.required_hours} ชม.</p>
                ...
                <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowAddModal(false)}>
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className="bg-gradient-to-r from-[#4472c4] to-[#032B68] px-8 py-6 text-white flex items-center justify-between">
                            ...
                            <button onClick={() => setShowAddModal(false)} className="p-2 hover:bg-white/20 rounded-xl"><X size={20} /></button>
                        </div>
                        <div className="p-6 overflow-y-auto max-h-[60vh]">
                            {loadingUnassigned ? (
                                <div className="text-center py-12 text-gray-400">กำลังโหลด...</div>
                            ) : unassigned.length === 0 ? (
                                <div className="text-center py-12 text-gray-400">ไม่มีนักศึกษาที่รอเพิ่ม</div>
                            ) : (
                                <div className="space-y-3">
                                    {unassigned.map((s: any) => (
                                        <div key={s.internship_id} className="flex items-center justify-between p-5 bg-gray-50 hover:bg-blue-50/50 rounded-2xl border border-gray-100">
                                            <div>
                                                <h3 className="font-bold text-gray-900">{s.student_code} {s.full_name}</h3>
                                                <p className="text-sm text-gray-500">{s.email || '-'}</p>
                                            </div>
                                            <button onClick={() => handleAssign(s.internship_id)} disabled={assigning === s.internship_id} className="px-5 py-2.5 bg-[#4472c4] hover:bg-[#365fa3] text-white font-bold rounded-xl disabled:opacity-50 flex items-center gap-2">
                                                <UserPlus size={16} />
