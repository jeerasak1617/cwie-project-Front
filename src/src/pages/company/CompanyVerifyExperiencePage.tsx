...
const CompanyVerifyExperiencePage = () => {
    ...
    const loadExperiences = async () => {
        setLoading(true);
        try { const res = await api.get(`/supervisor/experiences/${studentId}`); setExperiences(res.data.experiences || []); } catch {} finally { setLoading(false); }
    };

    const handleReview = async (expId: number) => {
        if (!comment.trim()) return;
        try { await api.post(`/supervisor/experiences/${expId}/review`, null, { params: { comment: comment.trim() } }); setComment(''); setReviewingId(null); loadExperiences(); } catch {}
    };
                    ...
                    {experiences.map((exp: any) => (
                        <div key={exp.id} className="bg-white rounded-2xl p-6 shadow-md">
                            <div className="flex justify-between items-start mb-3">
                                ...
                                {exp.supervisor_comment ? <span className="px-3 py-1 rounded-full text-xs font-bold bg-green-50 text-green-600">ตรวจแล้ว</span> : <span className="px-3 py-1 rounded-full text-xs font-bold bg-yellow-50 text-yellow-600">รอตรวจ</span>}
                            </div>
                            {exp.description && <p className="text-gray-600 text-sm mb-2">{exp.description}</p>}
                            {exp.supervisor_comment && <p className="text-orange-600 text-sm mt-3"><strong>ความเห็น:</strong> {exp.supervisor_comment}</p>}
                            {!exp.supervisor_comment && (
                                ...
                                ) : <button onClick={() => setReviewingId(exp.id)} className="mt-2 px-4 py-2 bg-orange-50 text-orange-600 rounded-full text-sm font-bold hover:bg-orange-100">ตรวจ + ให้ความเห็น</button>
                            )}
                        </div>
                    ))}
                </div>
